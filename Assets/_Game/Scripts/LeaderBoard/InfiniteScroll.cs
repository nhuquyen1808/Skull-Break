...
namespace Mopsicus.InfiniteScroll
{
    ...
    public class InfiniteScroll : MonoBehaviour, IDropHandler
    {
        ...
        /// Previous position
        /// </summary>
        public int _previousPosition = -1;
        ...
        [SerializeField] bool isForce = false;
        [SerializeField] bool isForceUpdate = false;
        /// <summary>
        /// Constructor
        /// </summary>
        public void Init(bool isFixSize = false)
        {

            var rtfm = GetComponent<RectTransform>();
       
            if (isFixSize)
                rtfm.sizeDelta = new Vector2(rtfm.sizeDelta.x, 900f / (((float)Screen.width / (float)Screen.height) / (1080f / 1920f)));
            
            _container = rtfm.rect;
        ...
        void UpdateVertical()
        {
            ...
            if (_rects != null && _rects[0] != null && _topPosition <= 0f && _rects[0].anchoredPosition.y < -TopPadding - 10f)
            {
                InitData(_count);
                return;
            }
        ...
        float CalcSizesPositionsVertical(int count)
        {
            if (_heights != null)
                _heights.Clear();
            _heights = new Dictionary<int, int>(count);
            if (_positions != null)
                _positions.Clear();
            _positions = new Dictionary<int, float>(count);
            float result = 0f;
        ...
        /// <param name="index">Index to move from</param>
        /// <param name="height">New height</param>
        public void MoveDataToVertical(int index, float height)
        {
            _content.sizeDelta = new Vector2(_content.sizeDelta.x, height);
            Vector2 pos = _content.anchoredPosition;
            for (int i = 0; i < _views.Length; i++)
        ...
        public void MoveToRealIndex(int realIndex, int count, float duration, UnityAction callback = null, int minOffset = 2)
        {
            isForce = true;
            // isForceUpdate = true;
            int index = realIndex - minOffset;
            _content.DOAnchorPos(new Vector3(0, Mathf.Clamp(index * (_heights[0] + ItemSpacing), 0, _content.sizeDelta.y - 2 * (_heights[0] + ItemSpacing)), 0), duration).OnUpdate(() =>
            {
                ForceUpdateVertical();
        ...
        public void ForceVerticleToRealIndex(int realIndex, int count,int minOffset =2)
        {
            _content.DOKill();
            isForce = true;

            int indexPos = realIndex - minOffset;

            _content.anchoredPosition = new Vector3(0, Mathf.Clamp(indexPos * (_heights[0] + ItemSpacing)+5f, 0, _content.sizeDelta.y - (_heights[0] + ItemSpacing) * (_views.Length - 4.5f)), 0);


            int bonus = Math.Min(3, count - 1 - realIndex);

            int valueChange = realIndex + bonus;

            int mul = valueChange / _views.Length;
            int countMax = valueChange - mul * _views.Length;


            int firstIndex = valueChange - _views.Length;
            if (firstIndex < 0)
            {
                firstIndex = 0;
            }
            ...
            else
            {
                ...
                {
                    int index = i + ((i <= countMax) ? mul : mul - 1) * _views.Length;
                    if (_positions.ContainsKey(index))
                        _rects[i].anchoredPosition = new Vector2(LeftPadding, _positions[index]);
                    _views[i].name = $"{index}";
                    OnFill(index, _views[i]);
                }
                _previousPosition = firstIndex + 1;
            }

            isForce = false;
        }

        public void RemoveAtIndex(float duration, int realIndex, UnityAction callBack = null)
        {
            isForce = true;
            int index = RealIndexToIndexView(realIndex);

            bool firstTime = true;

            int countId = 0;

            _rects[index].DOScale(Vector3.zero, duration).OnComplete(() =>
            {
                for (int i = index; i < _rects.Length; i++)
                {
                    var rect = _rects[i];
                    var itemView = _views[i];


                    var savePos = rect.anchoredPosition;
                    var newPos = rect.anchoredPosition + new Vector2(0, _heights[0] + ItemSpacing);



                    if (rect.anchoredPosition.y <= _rects[index].anchoredPosition.y)
                    {
                        rect.DOAnchorPos(newPos, 0f).OnComplete(() =>
                        {
                            if (firstTime)
                            {
                                firstTime = false;
                                _rects[index].localScale = Vector3.one;

                                callBack?.Invoke();
                            }
                            rect.anchoredPosition = savePos;
                            OnFill(realIndex + 1 + countId, itemView);
                            countId++;
                        });
                    }
                }
                for (int i = 0; i < index; i++)
                {
                    ...
                    if (rect.anchoredPosition.y <= _rects[index].anchoredPosition.y)
                    {
                        rect.DOAnchorPos(newPos, 0).OnComplete(() =>
                        {
                            if (firstTime)
                            {
                                firstTime = false;
                            ...
                            rect.DOAnchorPos(savePos, 0).OnComplete(() =>
                            {
                                OnFill(realIndex + 1 + countId, itemView);
                                countId++;
                            });
        ...
        public void InsertAtIndex(float duration, int realIndex, UnityAction callBack = null)
        {
            isForce = true;
            int index = RealIndexToIndexView(realIndex);
            ...
            for (int i = index; i < _rects.Length; i++)
            {
                ...
                if (rect.anchoredPosition.y <= _rects[index].anchoredPosition.y)
                {
                    rect.DOAnchorPos(newPos, duration).OnComplete(() =>
                    {
                        if (firstTime)
                        {
                            firstTime = false;
                        ...
                        rect.anchoredPosition = savePos;
                        OnFill(realIndex + countId, itemView);
                        countId++;
                    });
                }
            }
        ...
        public Vector3 GetCurrentIndexPos(int index)
        {
            return _rects[index].anchoredPosition;
        }
        public int RealIndexToIndexView(int realIndex)
        {
            int index = -1;
            for (int i = 0; i < _rects.Length; i++)
            {
                if (_positions[realIndex] == _rects[i].anchoredPosition.y)
                    index = i;
            }
        ...
        public float GetPosY(int realIndex)
        {
            return _positions[realIndex];
        }
        public void ReInitGlobalIndexItem(int globalIndex)
        {
            int index = RealIndexToIndexView(globalIndex);
            if (index == -1)
                return;

            OnFill(globalIndex, _views[index]);
        }
